...
__global__ void flash_attention_fwd(float *Q, float *K, float *V, float *O,
  ...
  for (int i = 0; i < Tr; i++) {
    ...
    for (int j = 0; j < Tc; j++) {
      ...
      S_ij[s_row * Bc + s_col] = acc * (1.0f / sqrtf(d));
      __syncthreads();
...
torch::Tensor fa2_forward(torch::Tensor Q, torch::Tensor K, torch::Tensor V) {
  ...
  flash_attention_fwd<Br, Bc><<<grid_size, block_size, smem_size>>>(
      ...
      O.data_ptr<float>(), L.data_ptr<float>(), N, d, Tr, Tc);
  return O;
}

__global__ void q_kt_mul(float *Q, float *K, float *O, int N, int d, int Br,
                         int Bc, int Tr, int Tc) {
  extern __shared__ float smem[];
  float *Q_i = smem;
  float *K_i = Q_i + (Br * d);
  float *S_ij = K_i + (Bc * d);
  ...
  // Tile Q
  for (int i = 0; i < Tr; i++) {
    for (int k = tx; k < Br * d; k += (Br * Bc)) {
      int row = k / d;
      int col = k % d;
      int global_row = i * Br + row;
      if (row < Br && col < Bc && global_row < N) {
        Q_i[row * d + col] = Q[qkv_offset + global_row * d + col];
        printf("\nIn i=%dth iteration => ThreadIdx.x %d Shared Idx Q_i[%d] = "
               "Q[%d](%f)\n",
               i, threadIdx.x, row * d + col, qkv_offset + global_row * d + col,
               Q[qkv_offset + global_row * d + col]);
      }
    }
    __syncthreads();
    for (int j = 0; j < Tc; j++) {
      for (int k = tx; k < Bc * d; k += (Br * Bc)) {
        int row = k / d;
        int col = k % d;
        int global_row = j * Bc + row;

        if (row < Bc && col < d && global_row < N) {
          int shared_idx = col * Bc + row; // transposed
          int global_idx = qkv_offset + global_row * d + col;
          K_i[col * d + row] = K[global_idx];
          printf(
              "\nIn j=%dth iteration => ThreadIdx.x %d K_i[%d] = K[%d](%f)\n",
              j, threadIdx.x, shared_idx, global_idx, K[global_idx]);
        }
      }
      __syncthreads();
    }
...
torch::Tensor q_kt_mul(torch::Tensor Q, torch::Tensor K) {
  ...
  int smem_size = 2 * (Br * d) + (Bc * d) + (Br * Bc);
  q_kt_mul<<<grid_size, block_size, smem_size>>>(
      Q.data_ptr<float>(), K.data_ptr<float>(), O.data_ptr<float>(), N, d, Br,
      Bc, Tr, Tc);
  return O;
}
