...
fa2_attention = load(name="fa2_attention", sources = ["bind.cpp", "fa2_complete.cu"], extra_cflags=["-g", "-O3"], extra_cuda_cflags=["-g", "-G", "-lineinfo", "--ptxas-options=-v"])
...
def manual_attention_backward(dO, Q, K, V, O, causal:bool):
    ...
    dK = torch.matmul(dS.transpose(-1, -2), Q) * softmax_scale

    return dQ, dK,P, softmax_scale, dS, dV 

def test_fa2_attention(Q, K, V, dO, causal):
    
    O_manual = manual_attention(Q, K, V, causal)

    O_cuda, M_cuda= fa2_attention.forward(Q, K, V, causal)

    dQ_manual, dK_manual, P_manual, softmax_scale, dS_manual, dV_manual = manual_attention_backward(dO, Q, K, V, O_manual, causal)

    dQ_cuda, dK_cuda, P_cuda, dS_cuda, dV_cuda = fa2_attention.backward(dO, Q, K, V, O_cuda, M_cuda, causal)
    ...
    if not torch.allclose(dS_manual, dS_cuda, atol=atol, rtol=rtol):
    ...
    if not torch.allclose(dK_manual, dK_cuda, atol=atol, rtol=rtol):
        print(f"dK_manual {dK_manual}\ndK_cuda {dK_cuda}\n")
    ...
    if not torch.allclose(dV_manual, dV_cuda, atol=atol, rtol=rtol):
        print(f"dV_manual {dK_manual}\ndV_cuda {dK_cuda}\n")
...
if __name__ == "__main__":
    ...
    test_fa2_attention(Q, K, V, dO,causal=False)
    test_fa2_attention(Q, K, V, dO,causal=True)
