...
__global__ void compute_dK_dV(const float *Q, const float *K, float *V,
                              ...
                              int batch_size, int num_heads, int seq_len,
                              int head_dim, int Bc, int Br, bool causal) {
  ...
  for (int j = 0; j < (seq_len + Br - 1) / Br; j++) {
    ...
    for (int q = threadIdx.y; q < Br && (q_tile_idx + q) < seq_len;
      ...
      for (int k = threadIdx.x; k < Bc && (kv_tile_idx + k) < seq_len;
        ...
        float qk = 0.0f;
        for (int d = 0; d < head_dim; d++) {
          qk += Q_tile[q * head_dim_padded + d] * K_tile[k * head_dim_padded + d];
        }
    ...
    for (int q = threadIdx.y; q < Br && (q_tile_idx + q) < seq_len;
      ...
      for (int k = threadIdx.x; k < Bc && (kv_tile_idx + k) < seq_len;
        ...
        for (int d = 0; d < head_dim; d++) {
          dp +=
              dO[qkv_offset + q_idx * head_dim_padded + d] * V_tile[k * head_dim_padded + d];
        }
...
__global__ void compute_dQ(const float *Q, const float *K, const float *V,
  ...
  extern __shared__ float smem[];
  float *Q_tile = smem;
  float *dO_tile = Q_tile + Br * head_dim_padded;
  float *dQ_tile = dO_tile + Br * head_dim_padded;
  float *K_tile = dQ_tile + Br * head_dim_padded;
  float *V_tile = K_tile + Bc * head_dim_padded;
  float *S = V_tile + Bc * head_dim_padded;
  float *dS = S + Br * Bc;
  // load Q and dO tiles
  for (int i = threadIdx.y; i < Br && (q_tile_idx + i) < seq_len;
       i += blockDim.y) {
    for (int d = threadIdx.x; d < head_dim; d += blockDim.x) {
      int idx = qkv_offset + (q_tile_idx + i) * head_dim + d;
      Q_tile[i * head_dim_padded + d] = Q[idx];
      dO_tile[i * head_dim_padded + d] = dO[idx];
      dQ_tile[i * head_dim_padded + d] = 0.0f;
    }
  }
  __syncthreads();
  ...
  for (int j = 0; j < (seq_len + Bc - 1) / Bc; j++) {
    ...
    for (int q = threadIdx.y; q < Br && (q_tile_idx + q) < seq_len;
      ...
      for (int k = threadIdx.x; k < Bc && (k_tile_idx + k) < seq_len;
        ...
        float dP = 0.0f;
        for (int d = 0; d < head_dim; d++) {
          dP += dO_tile[q * head_dim_padded + d] *
      ...
      for (int d = threadIdx.x; d < head_dim; d += blockDim.x) {
        ...
        for (int k = 0; k < Bc && (k_tile_idx + k) < seq_len; ++k) {
          if (!causal || (q_tile_idx + q) >= (k_tile_idx + k)) {
            dq += dS[q * Bc + k] * K_tile[k * head_dim_padded + d];
          }
        }
        dQ_tile[q * head_dim_padded + d] += dq * softmax_scale;
      }
    }
  }
  ...
  for (int i = threadIdx.y; i < Br && (q_tile_idx + i) < seq_len;
    ...
    for (int d = threadIdx.x; d < head_dim; d += blockDim.x) {
      int idx = qkv_offset + (q_tile_idx + i) * head_dim + d;
      atomicAdd(&dQ[idx], dQ_tile[i * head_dim_padded + d]);
    }
  }
}
...
fa2_backward(torch::Tensor dO, torch::Tensor Q, torch::Tensor K,
  ...
  size_t smem_size_dk_dv =
      (Bc * head_dim_padded * 2 + Br * head_dim_padded + 2 * Br * Bc) *
      sizeof(float);
  compute_dK_dV<<<grid_dK_dV, block_dK_dV, smem_size_dk_dv>>>(
      Q.data_ptr<float>(), K.data_ptr<float>(), V.data_ptr<float>(),
      dO.data_ptr<float>(), M.data_ptr<float>(), D, dK.data_ptr<float>(),
      dV.data_ptr<float>(), softmax_scale, batch_size, num_heads, seq_len,
      head_dim, Bc, Br, causal);
  CHECK_CUDA(cudaGetLastError());
  CHECK_CUDA(cudaDeviceSynchronize());
  ...
  dim3 block_dq(32, 16);
  size_t smem_size_dq =
      (Br * head_dim_padded * 3 + Bc * head_dim_padded * 2 + 2 * Br * Bc) *
      sizeof(float);
  compute_dQ<<<grid_dq, block_dq, smem_size_dq>>>(
      Q.data_ptr<float>(), K.data_ptr<float>(), V.data_ptr<float>(),
      dO.data_ptr<float>(), M.data_ptr<float>(), D, dQ.data_ptr<float>(),
      softmax_scale, batch_size, num_heads, seq_len, head_dim, Br, Bc, causal);
  CHECK_CUDA(cudaGetLastError());
  CHECK_CUDA(cudaDeviceSynchronize());
