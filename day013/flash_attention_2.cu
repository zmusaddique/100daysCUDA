...
__global__ void fa2_bwd(float *dQ, float *dK, float *dV, const float *Q,
  ...
  for (int j = 0; j < Tc; j++) {
    ...
    if (ty < Br) {
      ...
      if (q_valid) {
        for (int k = 0; k < Bc; k++) {
          ...
          if (k_valid && (!causal || q_global_idx >= k_global_idx)) {
            float ds_val = 0.0f;
            for (int d = 0; d < head_dim; d++) {
              ds_val += dO_tile[ty * head_dim_padded + d] *
                        V_tile[k * head_dim_padded + d];
            }
            dS[ty * Bc_padded + k] = ds_val;
          }
        }
      }
      __syncthreads();

      // Apply softmax backward: dS_ij = S_ij * (dS_ij - sum_k(S_ik * dS_ik))
      if (ty < Br) {
        const int q_global_idx = q_tile_idx + ty;
        const bool q_valid = q_global_idx < seq_len;
        ...
        if (q_valid) {
          float sum_S_dS = 0.0f;
          for (int k = 0; k < Bc; k++) {
            sum_S_dS += S[ty * Bc_padded + k] * dS[ty * Bc_padded + k];
          }

          for (int k = 0; k < Bc; k++) {
            dS[ty * Bc_padded + k] =
                S[ty * Bc_padded + k] * (dS[ty * Bc_padded + k] - sum_S_dS);
          }
        }
      }
      __syncthreads();

      // Compute dQ = dS * K for this tile and accumulate
      if (ty < Br) {
        const int q_global_idx = q_tile_idx + ty;
        const bool q_valid = q_global_idx < seq_len;
        ...
        if (q_valid) {
          for (int d = 0; d < head_dim; d++) {
            ...
            for (int k = 0; k < Bc; k++) {
              dq_val +=
                  dS[ty * Bc_padded + k] * K_tile[k * head_dim_padded + d];
            }
            dQ_tile[ty * head_dim_padded + d] += dq_val;
          }
        }
      }
      // Compute dK = Q^T * dS and update global memory atomically
      if (tx < head_dim) {
        for (int k = 0; k < Bc; k++) {
          const int k_global_idx = k_tile_idx + k;
          const bool k_valid = k_global_idx < seq_len;
          ...
          if (k_valid) {
            float dk_val = 0.0f;
            for (int q_idx = 0; q_idx < Br; q_idx++) {
              const int q_global_idx = q_tile_idx + q_idx;
              if (q_global_idx < seq_len &&
                  (!causal || q_global_idx >= k_global_idx)) {
                dk_val += Q_tile[q_idx * head_dim_padded + tx] *
                          dS[q_idx * Bc_padded + k];
              }
            }
            atomicAdd(&dK[qkv_offset + k_global_idx * head_dim + tx], dk_val);
          }
        }
        // Compute dV = S^T * dO and update global memory atomically
        if (tx < head_dim) {
          for (int k = 0; k < Bc; k++) {
            const int k_global_idx = k_tile_idx + k;
            const bool k_valid = k_global_idx < seq_len;
            ...
            if (k_valid) {
              ...
              for (int q_idx = 0; q_idx < Br; q_idx++) {
                ...
                if (q_global_idx < seq_len &&
                    (!causal || q_global_idx >= k_global_idx)) {
                  dv_val += S[q_idx * Bc_padded + k] *
                            dO_tile[q_idx * head_dim_padded + tx];
                }
              }
              atomicAdd(&dV[qkv_offset + k_global_idx * head_dim + tx], dv_val);
            }
          }
        }
        __syncthreads();
    ...
    for (int i = ty; i < Br; i += blockDim.y) {
      const int q_global_idx = q_tile_idx + i;
      const bool q_valid = q_global_idx < seq_len;

      if (q_valid) {
        const int base_idx = qkv_offset + q_global_idx * head_dim;
        for (int j = tx; j < head_dim; j += blockDim.x) {
          dQ[base_idx + j] = dQ_tile[i * head_dim_padded + j];
        }
      }
    }
  }
