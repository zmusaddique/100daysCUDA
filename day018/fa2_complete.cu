...
__global__ void compute_dK_dV(const float *Q, const float *K, float *V,
  ...
  float *P = Q_tile + Br * head_dim_padded;
  float *dS = P + Br * Bc;
  ...
  for (int j = 0; j < (seq_len + Br - 1) / Br; j++) {
    ...
    // Compute P = exp(softmax_scale * Q @ K^T - M)
    for (int q = threadIdx.y; q < Br && (q_tile_idx + q) < seq_len;
         q += blockDim.y) {
      ...
      for (int d = 0; d < head_dim; d++) {
        ...
        for (int q = 0; q < Br && (q_tile_idx + q) < seq_len; q++) {
          ...
          if (!causal || q_idx >= k_idx) {
            dv += P[q * Bc + k] * dO[qkv_offset + q_idx * head_dim + d];
          }
        }
...
__global__ void compute_dQ(const float *Q, const float *K, const float *V,
  ...
  for (int j = 0; j < (seq_len + Bc - 1) / Bc; j++) {
    ...
    // Write P to global mem
    for (int q = threadIdx.y; q < Br && (q_tile_idx + q) < seq_len;
         q += blockDim.y) {
      const int q_global = q_tile_idx + q;
      for (int k = threadIdx.x; k < Bc && (k_tile_idx + k) < seq_len;
           k += blockDim.x) {
        const int k_global = k_tile_idx + k;
        const int dS_global_idx =
            batch_head_idx * seq_len * seq_len + q_global * seq_len + k_global;
        P_g[dS_global_idx] += P[q * Bc + k];
      }
    }
    __syncthreads();
...
fa2_backward(torch::Tensor dO, torch::Tensor Q, torch::Tensor K,
  ...
  //    (Bc * head_dim_padded * 2 + 2 * Br * head_dim_padded + 2 * Br *
  //    Bc) *
  //      sizeof(float);
  //  compute_dK_dV<<<grid_dK_dV, block_dK_dV, smem_size_dk_dv>>>(
  //     Q.data_ptr<float>(), K.data_ptr<float>(), V.data_ptr<float>(),
  //   dO.data_ptr<float>(), M.data_ptr<float>(), L.data_ptr<float>(),
  //   D,
  //    dK.data_ptr<float>(), dV.data_ptr<float>(), softmax_scale,
  //    batch_size,
